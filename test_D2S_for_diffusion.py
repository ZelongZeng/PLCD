...
length2 = len(image_datasets['gallery_drone'])
if opt.low_altitude:
    length2 = len(image_datasets['gallery_drone_low'])
print(length1)
...
def extract_feature_h5(model,dataloaders, save_path):
    ...
    count = 0
    with h5py.File(save_path + '/pytorch_for_diffusion.hdf5', mode='w') as h5:
        with torch.no_grad():
            print('====> Extracting Gallery Descriptors')
            dbFeat = h5.create_dataset('gallery_feature',
                                       [length1, 512],
                                       dtype=np.float32)
        data_iterator_gallery_drone = tqdm(dataloaders['gallery_satellite'], desc='Gallery-satellite Testing...')
        ...
        data_iterator_query_street = tqdm(dataloaders['gallery_drone'], desc='Query-drone Testing...'.format(epoch))
        if opt.low_altitude:
            data_iterator_query_street = tqdm(dataloaders['gallery_drone_low'], desc='Query-drone Testing...'.format(epoch))
...
if __name__ == "__main__":
    ...
    scipy.io.savemat(save_path + '/pytorch_result_for_diffusion.mat',result)
