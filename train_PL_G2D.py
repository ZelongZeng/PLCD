...
image_datasets = {}
image_datasets['street'] = Multi_Folder(os.path.join(data_dir, 'street'),
...
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
               ...
               for x in ['street']}
dataset_sizes = {x: len(image_datasets[x]) for x in ['street']}
...
def train_model(model, model_teacher, model_test, criterion, optimizer, scheduler, num_epochs=25):
    ...
    warm_iteration = round(dataset_sizes['street'] / opt.batchsize) * opt.warm_epoch  # first 5 epoch
    ...
    for epoch in range(num_epochs - start_epoch):
        ...
        for phase in ['train']:
            ...
            data_iterator = tqdm(dataloaders['street'], desc='Epoch {} Training...'.format(epoch))
            ...
            for i, data in enumerate(data_iterator):
                ...
                pos = pos.view(6 * opt.batchsize, c, h, w) # for each anchor, we select 6 positives
                # copy pos 6 times
                ...
                for k in range(now_batch_size):
                    ...
                    score, rank = score.sort(dim=1, descending=opt.easy_pos)  # score high == easy
                ...
                if opt.gen != 0:
                    # knwoledge distillation for each region generated by R-MAC
                    final_part_f = torch.mm(f, pf.t())
                    final_part_f_teacher = torch.mm(f_teacher, pf_teacher.t())

                    if opt.hiera != 0:
                        for l in range(opt.hiera):
                            final_part_f = torch.cat((final_part_f, torch.mm(f, part_f[l].t())), 1)
                            final_part_f_teacher = torch.cat((final_part_f_teacher, torch.mm(f_teacher, part_f_teacher[l].t())), 1)

                    final_part_f_softmax = F.log_softmax(final_part_f / opt.tau, dim=1)
                    final_part_f_teacher_softmax = F.softmax(final_part_f_teacher / opt.tau, dim=1)

                    loss_part = kl_d(final_part_f_softmax, final_part_f_teacher_softmax.detach())
                ...
                if opt.gen != 0:
                    loss = opt.lambda1 * loss1 + opt.lambda2 * loss2 + opt.lambda3 * loss_part
                ...
                if int(version[0]) > 0 or int(version[2]) > 3:  # for the new version like 0.4.0, 0.5.0 and 1.0.0
                    ...
                    if opt.gen != 0:
                        running_loss3 += loss_part.item()
                else:  # for the old version like 0.3.0 and 0.3.1
                    running_loss += loss.data[0] * now_batch_size
                running_corrects += float(torch.sum(preds == labels.data))
                running_corrects2 += float(torch.sum(preds2 == pos_labels.data))

            epoch_loss = running_loss / dataset_sizes['street']
            epoch_loss1 = running_loss1 / dataset_sizes['street']
            epoch_loss3 = running_loss3 / dataset_sizes['street']
            epoch_loss2 = running_loss2 / dataset_sizes['street']
            epoch_acc = running_corrects / dataset_sizes['street']
            epoch_acc2 = running_corrects2 / dataset_sizes['street'] / 6

            print('{} Loss: {:.4f} Loss1: {:.4f} Loss2: {:.4f} Loss3: {:.4f} Street_Acc: '
...
model = two_view_net_rmac(len(class_names), droprate=opt.droprate, stride=opt.stride, pool=opt.pool,
                         share_weight=opt.share, gen=opt.gen)
if opt.gen != 0:
    model_teacher = two_view_net_rmac(len(class_names), droprate=opt.droprate, stride=opt.stride, pool=opt.pool,
                              share_weight=opt.share, gen=opt.gen)
...
if not opt.resume:
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
    # record every run
    copyfile('./train_PL_G2D.py', dir_name + '/train_PL_G2D.py')
